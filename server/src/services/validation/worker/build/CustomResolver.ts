...
import { Remapping } from "../../../../parser/common/types";
...
export class CustomResolver {
  constructor(
    ...
    private readonly _remappings: Remapping[]
  ) {}
  ...
  public async resolveSourceName(sourceName: string): Promise<ResolvedFile> {
    ...
    } else if (this._isRemapped(sourceName)) {
      return this._resolveRemappedSourceName(sourceName);
    } else {
      return this._resolveLibrarySourceName(sourceName);
    }
  }
  private _resolveRemappedSourceName(
    sourceName: string
  ): Promise<ResolvedFile> {
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    const remapping = this._findRemapping(sourceName)!;

    const absolutePath = sourceName.replace(remapping.from, remapping.to);

    return this._resolveFile(sourceName, absolutePath);
  }

  private _isRemapped(sourceName: string) {
    return this._findRemapping(sourceName) !== undefined;
  }

  private _findRemapping(sourceName: string) {
    return this._remappings.find(({ from }) => sourceName.startsWith(from));
  }
